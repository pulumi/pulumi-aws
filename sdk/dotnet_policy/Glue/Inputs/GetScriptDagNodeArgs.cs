// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Pulumi.PolicyPacks.Aws.Glue.Inputs
{
    public sealed class GetScriptDagNodeInputArgs
    {
        /// <summary>
        /// Nested configuration an argument or property of a node. Defined below.
        /// </summary>
        [Input("args")]
        #pragma warning disable CS0649 // Field is assigned through deserializer
        private global::Pulumi.Core.UndeferrableValue<List<Inputs.GetScriptDagNodeArgInputArgs>> _mArgs;

        public List<Inputs.GetScriptDagNodeArgInputArgs>? Args => _mArgs.GetValue("args");

        /// <summary>
        /// Node identifier that is unique within the node's graph.
        /// </summary>
        [Input("id")]
        #pragma warning disable CS0649 // Field is assigned through deserializer
        private global::Pulumi.Core.UndeferrableValue<string> _mId;

        public string? Id => _mId.GetValue("id");

        /// <summary>
        /// Line number of the node.
        /// </summary>
        [Input("lineNumber")]
        #pragma warning disable CS0649 // Field is assigned through deserializer
        private global::Pulumi.Core.UndeferrableValue<int> _mLineNumber;

        public int? LineNumber => _mLineNumber.GetValue("lineNumber");

        /// <summary>
        /// Type of node this is.
        /// </summary>
        [Input("nodeType")]
        #pragma warning disable CS0649 // Field is assigned through deserializer
        private global::Pulumi.Core.UndeferrableValue<string> _mNodeType;

        public string? NodeType => _mNodeType.GetValue("nodeType");
    }
}
