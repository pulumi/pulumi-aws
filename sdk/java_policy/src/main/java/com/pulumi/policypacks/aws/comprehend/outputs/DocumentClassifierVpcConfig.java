// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.policypacks.aws.comprehend.outputs;

import com.pulumi.core.UndeferrableValueException;
import com.pulumi.core.annotations.PolicyResourceProperty;
import java.lang.String;
import java.util.List;


public final class DocumentClassifierVpcConfig {

    /**
     * List of security group IDs.
     * 
     */
    @PolicyResourceProperty(name="securityGroupIds", flag="unknown_securityGroupIds")
    private List<String> value_securityGroupIds;
    private boolean unknown_securityGroupIds;
    public List<String> securityGroupIds() {
        if (!unknown_securityGroupIds) return value_securityGroupIds;
        throw new UndeferrableValueException("Value 'DocumentClassifierVpcConfig.securityGroupIds' is not present");
    }

    /**
     * List of VPC subnets.
     * 
     */
    @PolicyResourceProperty(name="subnets", flag="unknown_subnets")
    private List<String> value_subnets;
    private boolean unknown_subnets;
    public List<String> subnets() {
        if (!unknown_subnets) return value_subnets;
        throw new UndeferrableValueException("Value 'DocumentClassifierVpcConfig.subnets' is not present");
    }

}
