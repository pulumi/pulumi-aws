// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.policypacks.aws.customerprofiles.inputs;

import com.pulumi.core.UndeferrableValueException;
import com.pulumi.core.annotations.PolicyResourceProperty;
import java.lang.String;
import javax.annotation.Nullable;


public final class DomainMatchingExportingConfigS3ExportingArgs {

    /**
     * The name of the S3 bucket where Identity Resolution Jobs write result files.
     * 
     */
    @PolicyResourceProperty(name="s3BucketName", flag="unknown_s3BucketName")
    private String value_s3BucketName;
    private boolean unknown_s3BucketName;
    public String s3BucketName() {
        if (!unknown_s3BucketName) return value_s3BucketName;
        throw new UndeferrableValueException("Value 'DomainMatchingExportingConfigS3ExportingArgs.s3BucketName' is not present");
    }

    /**
     * The S3 key name of the location where Identity Resolution Jobs write result files.
     * 
     */
    @PolicyResourceProperty(name="s3KeyName", flag="unknown_s3KeyName")
    private String value_s3KeyName;
    private boolean unknown_s3KeyName;
    public String s3KeyName() {
        if (!unknown_s3KeyName) return value_s3KeyName;
        throw new UndeferrableValueException("Value 'DomainMatchingExportingConfigS3ExportingArgs.s3KeyName' is not present");
    }

}
