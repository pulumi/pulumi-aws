// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.aws.cloudfront.inputs;

import com.pulumi.core.annotations.Import;
import java.lang.String;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class GetOriginAccessIdentitiesPlainArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetOriginAccessIdentitiesPlainArgs Empty = new GetOriginAccessIdentitiesPlainArgs();

    /**
     * Filter origin access identities by comment.
     * 
     */
    @Import(name="comments")
    private @Nullable List<String> comments;

    /**
     * @return Filter origin access identities by comment.
     * 
     */
    public Optional<List<String>> comments() {
        return Optional.ofNullable(this.comments);
    }

    private GetOriginAccessIdentitiesPlainArgs() {}

    private GetOriginAccessIdentitiesPlainArgs(GetOriginAccessIdentitiesPlainArgs $) {
        this.comments = $.comments;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetOriginAccessIdentitiesPlainArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetOriginAccessIdentitiesPlainArgs $;

        public Builder() {
            $ = new GetOriginAccessIdentitiesPlainArgs();
        }

        public Builder(GetOriginAccessIdentitiesPlainArgs defaults) {
            $ = new GetOriginAccessIdentitiesPlainArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param comments Filter origin access identities by comment.
         * 
         * @return builder
         * 
         */
        public Builder comments(@Nullable List<String> comments) {
            $.comments = comments;
            return this;
        }

        /**
         * @param comments Filter origin access identities by comment.
         * 
         * @return builder
         * 
         */
        public Builder comments(String... comments) {
            return comments(List.of(comments));
        }

        public GetOriginAccessIdentitiesPlainArgs build() {
            return $;
        }
    }

}
