...
declare module "./function" {
    interface Function {
        /**
         * Creates an AWS/Lambda metric with the requested [metricName] for this specific Function.
         * See https://docs.aws.amazon.com/lambda/latest/dg/monitoring-functions-metrics.html for
         * list of all metric-names.
         *
         * Note, individual metrics can easily be obtained without supplying the name using the
         * other [metricXXX] functions.
         *
         * You can use the following dimensions to refine the metrics returned for your Lambda
         * functions:
         *
         * 1. "Resource". Filters the metric data by Lambda function resource, such as function
         *    version or alias.
         * 2. "ExecutedVersion". Filters the metric data by Lambda function versions. This only
         *    applies to alias invocations.
         */
        metric(metricName: metrics.LambdaMetricName, change?: cloudwatch.MetricChange): cloudwatch.Metric;
         ...
         */
        metricDuration(change?: cloudwatch.MetricChange): cloudwatch.Metric;
...
function getDimensions(lambda: LambdaFunction) {
    return { dimensions: { FunctionName: lambda.name } }
}
...
function getMetric(
    ...
    return moduleFunction(getDimensions(func)).with(change);
}

LambdaFunction.prototype.metric = function(this: LambdaFunction, metricName: metrics.LambdaMetricName, change: cloudwatch.MetricChange = {}) {
    return metrics.metric(metricName, getDimensions(this)).with(change);
}

LambdaFunction.prototype.metricInvocations = function(this: LambdaFunction, change: cloudwatch.MetricChange) {
    return getMetric(this, change, metrics.invocations);
}

LambdaFunction.prototype.metricErrors = function(this: LambdaFunction, change: cloudwatch.MetricChange) {
    return getMetric(this, change, metrics.errors);
}

LambdaFunction.prototype.metricDeadLetterErrors = function(this: LambdaFunction, change: cloudwatch.MetricChange) {
    return getMetric(this, change, metrics.deadLetterErrors);
}

LambdaFunction.prototype.metricDuration = function(this: LambdaFunction, change: cloudwatch.MetricChange) {
    return getMetric(this, change, metrics.duration);
}

LambdaFunction.prototype.metricThrottles = function(this: LambdaFunction, change: cloudwatch.MetricChange) {
    return getMetric(this, change, metrics.throttles);
}

LambdaFunction.prototype.metricIteratorAge = function(this: LambdaFunction, change: cloudwatch.MetricChange) {
    return getMetric(this, change, metrics.iteratorAge);
}
