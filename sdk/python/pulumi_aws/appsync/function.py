...
class Function(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if runtime is not None and not isinstance(runtime, FunctionRuntimeArgs):
                runtime = runtime or {}
                def _setter(key, value):
                    runtime[key] = value
                FunctionRuntimeArgs._configure(_setter, **runtime)
            __props__.__dict__["runtime"] = runtime
            if sync_config is not None and not isinstance(sync_config, FunctionSyncConfigArgs):
                sync_config = sync_config or {}
                def _setter(key, value):
                    sync_config[key] = value
                FunctionSyncConfigArgs._configure(_setter, **sync_config)
