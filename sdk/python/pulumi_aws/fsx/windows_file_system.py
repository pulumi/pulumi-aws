...
class WindowsFileSystemArgs:
    ...
    def _configure(
        ...
        if 'selfManagedActiveDirectory' in kwargs:
            self_managed_active_directory = kwargs['selfManagedActiveDirectory']
        if 'skipFinalBackup' in kwargs:
...
class _WindowsFileSystemState:
    ...
    def _configure(
        ...
        if 'preferredFileServerIp' in kwargs:
            preferred_file_server_ip = kwargs['preferredFileServerIp']
        if 'preferredSubnetId' in kwargs:
            preferred_subnet_id = kwargs['preferredSubnetId']
        if 'remoteAdministrationEndpoint' in kwargs:
...
class WindowsFileSystem(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if audit_log_configuration is not None and not isinstance(audit_log_configuration, WindowsFileSystemAuditLogConfigurationArgs):
                audit_log_configuration = audit_log_configuration or {}
                def _setter(key, value):
                    audit_log_configuration[key] = value
                WindowsFileSystemAuditLogConfigurationArgs._configure(_setter, **audit_log_configuration)
            ...
            if disk_iops_configuration is not None and not isinstance(disk_iops_configuration, WindowsFileSystemDiskIopsConfigurationArgs):
                disk_iops_configuration = disk_iops_configuration or {}
                def _setter(key, value):
                    disk_iops_configuration[key] = value
                WindowsFileSystemDiskIopsConfigurationArgs._configure(_setter, **disk_iops_configuration)
            ...
            if self_managed_active_directory is not None and not isinstance(self_managed_active_directory, WindowsFileSystemSelfManagedActiveDirectoryArgs):
                self_managed_active_directory = self_managed_active_directory or {}
                def _setter(key, value):
                    self_managed_active_directory[key] = value
                WindowsFileSystemSelfManagedActiveDirectoryArgs._configure(_setter, **self_managed_active_directory)
