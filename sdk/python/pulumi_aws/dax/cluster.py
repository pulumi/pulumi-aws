...
class _ClusterState:
    ...
    def _configure(
        ...
        if 'availabilityZones' in kwargs:
            availability_zones = kwargs['availabilityZones']
        if 'clusterAddress' in kwargs:
            cluster_address = kwargs['clusterAddress']
        if 'clusterEndpointEncryptionType' in kwargs:
            cluster_endpoint_encryption_type = kwargs['clusterEndpointEncryptionType']
        if 'clusterName' in kwargs:
            cluster_name = kwargs['clusterName']
        if 'configurationEndpoint' in kwargs:
...
class Cluster(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if server_side_encryption is not None and not isinstance(server_side_encryption, ClusterServerSideEncryptionArgs):
                server_side_encryption = server_side_encryption or {}
                def _setter(key, value):
                    server_side_encryption[key] = value
                ClusterServerSideEncryptionArgs._configure(_setter, **server_side_encryption)
