...
class ResourceLfTags(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if database is not None and not isinstance(database, ResourceLfTagsDatabaseArgs):
                database = database or {}
                def _setter(key, value):
                    database[key] = value
                ResourceLfTagsDatabaseArgs._configure(_setter, **database)
            ...
            if table is not None and not isinstance(table, ResourceLfTagsTableArgs):
                table = table or {}
                def _setter(key, value):
                    table[key] = value
                ResourceLfTagsTableArgs._configure(_setter, **table)
            __props__.__dict__["table"] = table
            if table_with_columns is not None and not isinstance(table_with_columns, ResourceLfTagsTableWithColumnsArgs):
                table_with_columns = table_with_columns or {}
                def _setter(key, value):
                    table_with_columns[key] = value
                ResourceLfTagsTableWithColumnsArgs._configure(_setter, **table_with_columns)
