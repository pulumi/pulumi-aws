...
class _DomainState:
    ...
    def _configure(
        ...
        if 'domainSettings' in kwargs:
            domain_settings = kwargs['domainSettings']
        if 'homeEfsFileSystemId' in kwargs:
            home_efs_file_system_id = kwargs['homeEfsFileSystemId']
        if 'kmsKeyId' in kwargs:
            kms_key_id = kwargs['kmsKeyId']
        if 'retentionPolicy' in kwargs:
            retention_policy = kwargs['retentionPolicy']
        if 'securityGroupIdForDomainBoundary' in kwargs:
            security_group_id_for_domain_boundary = kwargs['securityGroupIdForDomainBoundary']
        if 'singleSignOnManagedApplicationInstanceId' in kwargs:
...
class Domain(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if default_user_settings is not None and not isinstance(default_user_settings, DomainDefaultUserSettingsArgs):
                default_user_settings = default_user_settings or {}
                def _setter(key, value):
                    default_user_settings[key] = value
                DomainDefaultUserSettingsArgs._configure(_setter, **default_user_settings)
            ...
            if domain_settings is not None and not isinstance(domain_settings, DomainDomainSettingsArgs):
                domain_settings = domain_settings or {}
                def _setter(key, value):
                    domain_settings[key] = value
                DomainDomainSettingsArgs._configure(_setter, **domain_settings)
            __props__.__dict__["domain_settings"] = domain_settings
            __props__.__dict__["kms_key_id"] = kms_key_id
            if retention_policy is not None and not isinstance(retention_policy, DomainRetentionPolicyArgs):
                retention_policy = retention_policy or {}
                def _setter(key, value):
                    retention_policy[key] = value
                DomainRetentionPolicyArgs._configure(_setter, **retention_policy)
