...
class Workforce(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if cognito_config is not None and not isinstance(cognito_config, WorkforceCognitoConfigArgs):
                cognito_config = cognito_config or {}
                def _setter(key, value):
                    cognito_config[key] = value
                WorkforceCognitoConfigArgs._configure(_setter, **cognito_config)
            __props__.__dict__["cognito_config"] = cognito_config
            if oidc_config is not None and not isinstance(oidc_config, WorkforceOidcConfigArgs):
                oidc_config = oidc_config or {}
                def _setter(key, value):
                    oidc_config[key] = value
                WorkforceOidcConfigArgs._configure(_setter, **oidc_config)
            __props__.__dict__["oidc_config"] = oidc_config
            if source_ip_config is not None and not isinstance(source_ip_config, WorkforceSourceIpConfigArgs):
                source_ip_config = source_ip_config or {}
                def _setter(key, value):
                    source_ip_config[key] = value
                WorkforceSourceIpConfigArgs._configure(_setter, **source_ip_config)
            ...
            if workforce_vpc_config is not None and not isinstance(workforce_vpc_config, WorkforceWorkforceVpcConfigArgs):
                workforce_vpc_config = workforce_vpc_config or {}
                def _setter(key, value):
                    workforce_vpc_config[key] = value
                WorkforceWorkforceVpcConfigArgs._configure(_setter, **workforce_vpc_config)
