...
class FunctionArgs:
    ...
    def _configure(
        ...
        if replace_security_groups_on_destroy is None and 'replaceSecurityGroupsOnDestroy' in kwargs:
            replace_security_groups_on_destroy = kwargs['replaceSecurityGroupsOnDestroy']
        if replacement_security_group_ids is None and 'replacementSecurityGroupIds' in kwargs:
            replacement_security_group_ids = kwargs['replacementSecurityGroupIds']
...
class _FunctionState:
    ...
    def _configure(
        ...
        if package_type is None and 'packageType' in kwargs:
            package_type = kwargs['packageType']
        if qualified_arn is None and 'qualifiedArn' in kwargs:
            qualified_arn = kwargs['qualifiedArn']
        if qualified_invoke_arn is None and 'qualifiedInvokeArn' in kwargs:
            qualified_invoke_arn = kwargs['qualifiedInvokeArn']
        if replace_security_groups_on_destroy is None and 'replaceSecurityGroupsOnDestroy' in kwargs:
            replace_security_groups_on_destroy = kwargs['replaceSecurityGroupsOnDestroy']
        if replacement_security_group_ids is None and 'replacementSecurityGroupIds' in kwargs:
            replacement_security_group_ids = kwargs['replacementSecurityGroupIds']
        ...
        if s3_object_version is None and 's3ObjectVersion' in kwargs:
            s3_object_version = kwargs['s3ObjectVersion']
        if signing_job_arn is None and 'signingJobArn' in kwargs:
            signing_job_arn = kwargs['signingJobArn']
        if signing_profile_version_arn is None and 'signingProfileVersionArn' in kwargs:
            signing_profile_version_arn = kwargs['signingProfileVersionArn']
        if skip_destroy is None and 'skipDestroy' in kwargs:
            skip_destroy = kwargs['skipDestroy']
        if snap_start is None and 'snapStart' in kwargs:
            snap_start = kwargs['snapStart']
        if source_code_hash is None and 'sourceCodeHash' in kwargs:
            source_code_hash = kwargs['sourceCodeHash']
        if source_code_size is None and 'sourceCodeSize' in kwargs:
            source_code_size = kwargs['sourceCodeSize']
...
class Function(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            file_system_config = _utilities.configure(file_system_config, FunctionFileSystemConfigArgs, True)
