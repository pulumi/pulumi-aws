...
class Application(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if application_configuration is not None and not isinstance(application_configuration, ApplicationApplicationConfigurationArgs):
                application_configuration = application_configuration or {}
                def _setter(key, value):
                    application_configuration[key] = value
                ApplicationApplicationConfigurationArgs._configure(_setter, **application_configuration)
            __props__.__dict__["application_configuration"] = application_configuration
            if cloudwatch_logging_options is not None and not isinstance(cloudwatch_logging_options, ApplicationCloudwatchLoggingOptionsArgs):
                cloudwatch_logging_options = cloudwatch_logging_options or {}
                def _setter(key, value):
                    cloudwatch_logging_options[key] = value
                ApplicationCloudwatchLoggingOptionsArgs._configure(_setter, **cloudwatch_logging_options)
