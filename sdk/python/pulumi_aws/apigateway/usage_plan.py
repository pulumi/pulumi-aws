...
class UsagePlan(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if quota_settings is not None and not isinstance(quota_settings, UsagePlanQuotaSettingsArgs):
                quota_settings = quota_settings or {}
                def _setter(key, value):
                    quota_settings[key] = value
                UsagePlanQuotaSettingsArgs._configure(_setter, **quota_settings)
            __props__.__dict__["quota_settings"] = quota_settings
            __props__.__dict__["tags"] = tags
            if throttle_settings is not None and not isinstance(throttle_settings, UsagePlanThrottleSettingsArgs):
                throttle_settings = throttle_settings or {}
                def _setter(key, value):
                    throttle_settings[key] = value
                UsagePlanThrottleSettingsArgs._configure(_setter, **throttle_settings)
