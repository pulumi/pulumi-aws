...
class DevEnvironment(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if ides is not None and not isinstance(ides, DevEnvironmentIdesArgs):
                ides = ides or {}
                def _setter(key, value):
                    ides[key] = value
                DevEnvironmentIdesArgs._configure(_setter, **ides)
            ...
            if persistent_storage is not None and not isinstance(persistent_storage, DevEnvironmentPersistentStorageArgs):
                persistent_storage = persistent_storage or {}
                def _setter(key, value):
                    persistent_storage[key] = value
                DevEnvironmentPersistentStorageArgs._configure(_setter, **persistent_storage)
