...
class PresetArgs:
    ...
    def _configure(
             ...
             **kwargs):
        if 'audioCodecOptions' in kwargs:
            audio_codec_options = kwargs['audioCodecOptions']
        if 'videoCodecOptions' in kwargs:
            video_codec_options = kwargs['videoCodecOptions']
        if 'videoWatermarks' in kwargs:
...
class Preset(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if audio is not None and not isinstance(audio, PresetAudioArgs):
                audio = audio or {}
                def _setter(key, value):
                    audio[key] = value
                PresetAudioArgs._configure(_setter, **audio)
            __props__.__dict__["audio"] = audio
            if audio_codec_options is not None and not isinstance(audio_codec_options, PresetAudioCodecOptionsArgs):
                audio_codec_options = audio_codec_options or {}
                def _setter(key, value):
                    audio_codec_options[key] = value
                PresetAudioCodecOptionsArgs._configure(_setter, **audio_codec_options)
            ...
            if thumbnails is not None and not isinstance(thumbnails, PresetThumbnailsArgs):
                thumbnails = thumbnails or {}
                def _setter(key, value):
                    thumbnails[key] = value
                PresetThumbnailsArgs._configure(_setter, **thumbnails)
            __props__.__dict__["thumbnails"] = thumbnails
            __props__.__dict__["type"] = type
            if video is not None and not isinstance(video, PresetVideoArgs):
                video = video or {}
                def _setter(key, value):
                    video[key] = value
                PresetVideoArgs._configure(_setter, **video)
