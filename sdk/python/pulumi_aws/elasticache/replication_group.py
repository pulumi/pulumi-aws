...
class _ReplicationGroupState:
    ...
    def _configure(
        ...
        if maintenance_window is None and 'maintenanceWindow' in kwargs:
            maintenance_window = kwargs['maintenanceWindow']
        if member_clusters is None and 'memberClusters' in kwargs:
        ...
        if preferred_cache_cluster_azs is None and 'preferredCacheClusterAzs' in kwargs:
            preferred_cache_cluster_azs = kwargs['preferredCacheClusterAzs']
        if primary_endpoint_address is None and 'primaryEndpointAddress' in kwargs:
            primary_endpoint_address = kwargs['primaryEndpointAddress']
        if reader_endpoint_address is None and 'readerEndpointAddress' in kwargs:
            reader_endpoint_address = kwargs['readerEndpointAddress']
        if replicas_per_node_group is None and 'replicasPerNodeGroup' in kwargs:
            replicas_per_node_group = kwargs['replicasPerNodeGroup']
        if replication_group_id is None and 'replicationGroupId' in kwargs:
            replication_group_id = kwargs['replicationGroupId']
