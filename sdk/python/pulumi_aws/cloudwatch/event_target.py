...
class EventTarget(pulumi.CustomResource):
    ...
    Parameters used when you are using the rule to invoke an Amazon Batch Job. Documented below. A maximum of 1 are allowed.
      ...
      * `array_size` (`float`) - The size of the array, if this is an array batch job. Valid values are integers between 2 and 10,000.
      * `job_attempts` (`float`) - The number of times to attempt to retry, if the job fails. Valid values are 1 to 10.
    ...
    Parameters used when you are using the rule to invoke Amazon ECS Task. Documented below. A maximum of 1 are allowed.
      ...
      * `network_configuration` (`dict`) - Use this if the ECS task uses the awsvpc network mode. This specifies the VPC subnets and security groups associated with the task, and whether a public IP address is to be used. Required if launch_type is FARGATE because the awsvpc mode is required for Fargate tasks.
        * `assign_public_ip` (`bool`) - Assign a public IP address to the ENI (Fargate launch type only). Valid values are `true` or `false`. Default `false`.
      ...
      * `task_count` (`float`) - The number of tasks to create based on the TaskDefinition. The default is 1.
      * `task_definition_arn` (`str`) - The ARN of the task definition to use if the event target is an Amazon ECS cluster.
    """
    ...
    Parameters used when you are providing a custom input to a target based on certain event data.
      ...
      * `input_paths` (`dict`) - Key value pairs specified in the form of JSONPath (for example, time = $.time)
      * `input_template` (`str`) - Structure containing the template body.
    """
    kinesis_target: pulumi.Output[dict]
    """
    Parameters used when you are using the rule to invoke an Amazon Kinesis Stream. Documented below. A maximum of 1 are allowed.

      * `partition_key_path` (`str`) - The JSON path to be extracted from the event and used as the partition key.
    """
    ...
    def __init__(__self__, resource_name, opts=None, arn=None, batch_target=None, ecs_target=None, input=None, input_path=None, input_transformer=None, kinesis_target=None, role_arn=None, rule=None, run_command_targets=None, sqs_target=None, target_id=None, __props__=None, __name__=None, __opts__=None):
        ...
        The **batch_target** object supports the following:
          ...
          * `array_size` (`pulumi.Input[float]`) - The size of the array, if this is an array batch job. Valid values are integers between 2 and 10,000.
          * `job_attempts` (`pulumi.Input[float]`) - The number of times to attempt to retry, if the job fails. Valid values are 1 to 10.
          * `job_definition` (`pulumi.Input[str]`) - The ARN or name of the job definition to use if the event target is an AWS Batch job. This job definition must already exist.
          * `job_name` (`pulumi.Input[str]`) - The name to use for this execution of the job, if the target is an AWS Batch job.
        ...
        The **ecs_target** object supports the following:
          ...
          * `network_configuration` (`pulumi.Input[dict]`) - Use this if the ECS task uses the awsvpc network mode. This specifies the VPC subnets and security groups associated with the task, and whether a public IP address is to be used. Required if launch_type is FARGATE because the awsvpc mode is required for Fargate tasks.
            * `assign_public_ip` (`pulumi.Input[bool]`) - Assign a public IP address to the ENI (Fargate launch type only). Valid values are `true` or `false`. Default `false`.
          ...
          * `task_count` (`pulumi.Input[float]`) - The number of tasks to create based on the TaskDefinition. The default is 1.
          * `task_definition_arn` (`pulumi.Input[str]`) - The ARN of the task definition to use if the event target is an Amazon ECS cluster.

        The **input_transformer** object supports the following:

          * `input_paths` (`pulumi.Input[dict]`) - Key value pairs specified in the form of JSONPath (for example, time = $.time)
          * `input_template` (`pulumi.Input[str]`) - Structure containing the template body.

        The **kinesis_target** object supports the following:

          * `partition_key_path` (`pulumi.Input[str]`) - The JSON path to be extracted from the event and used as the partition key.
        ...
        The **sqs_target** object supports the following:
          ...
          * `message_group_id` (`pulumi.Input[str]`) - The FIFO message group ID to use as the target.
        """
