...
class StaticWebLayer(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if cloudwatch_configuration is not None and not isinstance(cloudwatch_configuration, StaticWebLayerCloudwatchConfigurationArgs):
                cloudwatch_configuration = cloudwatch_configuration or {}
                def _setter(key, value):
                    cloudwatch_configuration[key] = value
                StaticWebLayerCloudwatchConfigurationArgs._configure(_setter, **cloudwatch_configuration)
            ...
            if load_based_auto_scaling is not None and not isinstance(load_based_auto_scaling, StaticWebLayerLoadBasedAutoScalingArgs):
                load_based_auto_scaling = load_based_auto_scaling or {}
                def _setter(key, value):
                    load_based_auto_scaling[key] = value
                StaticWebLayerLoadBasedAutoScalingArgs._configure(_setter, **load_based_auto_scaling)
