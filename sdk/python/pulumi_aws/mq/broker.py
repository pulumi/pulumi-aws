...
class Broker(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if configuration is not None and not isinstance(configuration, BrokerConfigurationArgs):
                configuration = configuration or {}
                def _setter(key, value):
                    configuration[key] = value
                BrokerConfigurationArgs._configure(_setter, **configuration)
            ...
            if logs is not None and not isinstance(logs, BrokerLogsArgs):
                logs = logs or {}
                def _setter(key, value):
                    logs[key] = value
                BrokerLogsArgs._configure(_setter, **logs)
            __props__.__dict__["logs"] = logs
            if maintenance_window_start_time is not None and not isinstance(maintenance_window_start_time, BrokerMaintenanceWindowStartTimeArgs):
                maintenance_window_start_time = maintenance_window_start_time or {}
                def _setter(key, value):
                    maintenance_window_start_time[key] = value
                BrokerMaintenanceWindowStartTimeArgs._configure(_setter, **maintenance_window_start_time)
