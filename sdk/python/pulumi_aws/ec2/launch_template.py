...
class LaunchTemplate(pulumi.CustomResource):
    ...
    See Block Devices below for details.
      ...
      * `ebs` (`dict`) - Configure EBS volume properties.
        ...
        * `kms_key_id` (`str`) - The ARN of the AWS Key Management Service (AWS KMS) customer main key (CMK) to use when creating the encrypted volume.
        ...
        The **block_device_mappings** object supports the following:
          ...
          * `ebs` (`pulumi.Input[dict]`) - Configure EBS volume properties.
            ...
            * `iops` (`pulumi.Input[float]`) - The amount of provisioned
              [IOPS](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ebs-io-characteristics.html).
              This must be set with a `volume_type` of `"io1"`.
            * `kms_key_id` (`pulumi.Input[str]`) - The ARN of the AWS Key Management Service (AWS KMS) customer main key (CMK) to use when creating the encrypted volume.
              `encrypted` must be set to `true` when this is set.
