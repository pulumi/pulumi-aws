...
class _VpnConnectionState:
    ...
    def _configure(
        ...
        if 'coreNetworkAttachmentArn' in kwargs:
            core_network_attachment_arn = kwargs['coreNetworkAttachmentArn']
        if 'customerGatewayConfiguration' in kwargs:
        ...
        if 'transportTransitGatewayAttachmentId' in kwargs:
            transport_transit_gateway_attachment_id = kwargs['transportTransitGatewayAttachmentId']
        if 'tunnel1Address' in kwargs:
            tunnel1_address = kwargs['tunnel1Address']
        if 'tunnel1BgpAsn' in kwargs:
            tunnel1_bgp_asn = kwargs['tunnel1BgpAsn']
        if 'tunnel1BgpHoldtime' in kwargs:
            tunnel1_bgp_holdtime = kwargs['tunnel1BgpHoldtime']
        if 'tunnel1CgwInsideAddress' in kwargs:
        ...
        if 'tunnel2StartupAction' in kwargs:
            tunnel2_startup_action = kwargs['tunnel2StartupAction']
        if 'tunnel2VgwInsideAddress' in kwargs:
            tunnel2_vgw_inside_address = kwargs['tunnel2VgwInsideAddress']
        if 'tunnelInsideIpVersion' in kwargs:
            tunnel_inside_ip_version = kwargs['tunnelInsideIpVersion']
        if 'vgwTelemetries' in kwargs:
...
class VpnConnection(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if tunnel1_log_options is not None and not isinstance(tunnel1_log_options, VpnConnectionTunnel1LogOptionsArgs):
                tunnel1_log_options = tunnel1_log_options or {}
                def _setter(key, value):
                    tunnel1_log_options[key] = value
                VpnConnectionTunnel1LogOptionsArgs._configure(_setter, **tunnel1_log_options)
            ...
            if tunnel2_log_options is not None and not isinstance(tunnel2_log_options, VpnConnectionTunnel2LogOptionsArgs):
                tunnel2_log_options = tunnel2_log_options or {}
                def _setter(key, value):
                    tunnel2_log_options[key] = value
                VpnConnectionTunnel2LogOptionsArgs._configure(_setter, **tunnel2_log_options)
