...
class _InstanceState:
    ...
    def _configure(
        ...
        if 'instanceInitiatedShutdownBehavior' in kwargs:
            instance_initiated_shutdown_behavior = kwargs['instanceInitiatedShutdownBehavior']
        if 'instanceLifecycle' in kwargs:
        ...
        if 'sourceDestCheck' in kwargs:
            source_dest_check = kwargs['sourceDestCheck']
        if 'spotInstanceRequestId' in kwargs:
...
class Instance(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if capacity_reservation_specification is not None and not isinstance(capacity_reservation_specification, InstanceCapacityReservationSpecificationArgs):
                capacity_reservation_specification = capacity_reservation_specification or {}
                def _setter(key, value):
                    capacity_reservation_specification[key] = value
                InstanceCapacityReservationSpecificationArgs._configure(_setter, **capacity_reservation_specification)
            ...
            if instance_market_options is not None and not isinstance(instance_market_options, InstanceInstanceMarketOptionsArgs):
                instance_market_options = instance_market_options or {}
                def _setter(key, value):
                    instance_market_options[key] = value
                InstanceInstanceMarketOptionsArgs._configure(_setter, **instance_market_options)
            ...
            if launch_template is not None and not isinstance(launch_template, InstanceLaunchTemplateArgs):
                launch_template = launch_template or {}
                def _setter(key, value):
                    launch_template[key] = value
                InstanceLaunchTemplateArgs._configure(_setter, **launch_template)
            __props__.__dict__["launch_template"] = launch_template
            if maintenance_options is not None and not isinstance(maintenance_options, InstanceMaintenanceOptionsArgs):
                maintenance_options = maintenance_options or {}
                def _setter(key, value):
                    maintenance_options[key] = value
                InstanceMaintenanceOptionsArgs._configure(_setter, **maintenance_options)
            __props__.__dict__["maintenance_options"] = maintenance_options
            if metadata_options is not None and not isinstance(metadata_options, InstanceMetadataOptionsArgs):
                metadata_options = metadata_options or {}
                def _setter(key, value):
                    metadata_options[key] = value
                InstanceMetadataOptionsArgs._configure(_setter, **metadata_options)
            ...
            if root_block_device is not None and not isinstance(root_block_device, InstanceRootBlockDeviceArgs):
                root_block_device = root_block_device or {}
                def _setter(key, value):
                    root_block_device[key] = value
                InstanceRootBlockDeviceArgs._configure(_setter, **root_block_device)
