...
class ClusterS3Import(dict):
    ...
    def _configure(
             ...
             ingestion_role: Optional[str] = None,
...
class GetClusterMasterUserSecretResult(dict):
    ...
    def _configure(
        ...
        if secret_status is None and 'secretStatus' in kwargs:
            secret_status = kwargs['secretStatus']
        if secret_status is None:
            raise TypeError("Missing 'secret_status' argument")
...
class GetProxyAuthResult(dict):
    ...
    def _configure(
        ...
        if auth_scheme is None:
            raise TypeError("Missing 'auth_scheme' argument")
        if client_password_auth_type is None and 'clientPasswordAuthType' in kwargs:
            client_password_auth_type = kwargs['clientPasswordAuthType']
        if client_password_auth_type is None:
            raise TypeError("Missing 'client_password_auth_type' argument")
        if description is None:
            raise TypeError("Missing 'description' argument")
        if iam_auth is None and 'iamAuth' in kwargs:
            iam_auth = kwargs['iamAuth']
        if iam_auth is None:
            raise TypeError("Missing 'iam_auth' argument")
        if secret_arn is None and 'secretArn' in kwargs:
            secret_arn = kwargs['secretArn']
        if secret_arn is None:
            raise TypeError("Missing 'secret_arn' argument")
