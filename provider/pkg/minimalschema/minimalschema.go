...
func NewMinimalSchema(realSchema schema.PackageSpec) *MinimalSchema {
	...
	return &MinimalSchema{result}
}

func (ls *MinimalSchema) isDanglingRef(ref string) bool {
	switch {
	case strings.HasPrefix(ref, "#/types/"):
		clean := strings.TrimPrefix(ref, "#/types/")
		if _, ok := ls.spec.Types[clean]; !ok {
			return true
		}
		return false
...
func (ls *MinimalSchema) isTypeString(m map[string]any) bool {
	typ, ok := m["type"]
	if !ok {
		return false
	}
...
func (ls *MinimalSchema) isDanglingPlainRef(m map[string]any) bool {
	r, ok := m["$ref"]
	if !ok {
		return false
	}
...
func (ls *MinimalSchema) isDanglingStringRef(m map[string]any) bool {
...
//			]
//		}
func (ls *MinimalSchema) isUnionStringDanglingRef(m map[string]any) bool {
	if !ls.isTypeString(m) {
		return false
	}
	oneOf, ok := m["oneOf"]
...
//		]
//	}
func (ls *MinimalSchema) isUnion2DanglingRefs(m map[string]any) bool {
	oneOf, ok := m["oneOf"]
	if !ok {
		return false
	}
...
func (ls *MinimalSchema) rewrite(schema any) any {
...
func (ls *MinimalSchema) Write(destinationFile string) {
	copy := ls.spec
	copy.Version = ""
