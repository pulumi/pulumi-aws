...
const subnet0 =new aws.ec2.Subnet("test0", {
    vpcId: vpc.id,
    availabilityZone: aws.getAvailabilityZones(undefined, { ...providerOpts, async: true }).then(azs => azs.names[0]),
...
const subnet1 = new aws.ec2.Subnet("test1", {
    vpcId: vpc.id,
    availabilityZone: aws.getAvailabilityZones(undefined, { ...providerOpts, async: true }).then(azs => azs.names[1]),
