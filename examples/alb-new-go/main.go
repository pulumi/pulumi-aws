...
import (
	...
	"github.com/pulumi/pulumi-aws/sdk/go/aws/elasticloadbalancingv2"
...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		vpc, err := ec2.NewVpc(ctx, "text", &ec2.VpcArgs{
			CidrBlock:          pulumi.String("10.12.0.0/16"),
		...
		subnet0, err := ec2.NewSubnet(ctx, "test0", &ec2.SubnetArgs{
			...
			CidrBlock:           pulumi.String("10.12.0.0/24"),
		...
		subnet1, err := ec2.NewSubnet(ctx, "test1", &ec2.SubnetArgs{
			...
			CidrBlock:           pulumi.String("10.12.1.0/24"),
			MapPublicIpOnLaunch: pulumi.Bool(true),
		}, providerOpts...)
		if err != nil {
			return err
		...
		_, err = elasticloadbalancingv2.NewLoadBalancer(ctx, "test", &elasticloadbalancingv2.LoadBalancerArgs{
