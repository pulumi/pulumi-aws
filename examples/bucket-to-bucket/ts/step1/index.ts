...
const loggingBucket = new aws.s3.Bucket("loggingBucket", {
    forceDestroy: true,
    // NOTE: You would normally have to add ACLs using the sidecar resource
    // but this is specifically testing this field on Bucket
    // will fail one first up until the ownership controls are set
    // grants: [
    //     {
    //         permissions: ['FULL_CONTROL'],
    //         type: 'CanonicalUser',
    //         id: aws.s3.getCanonicalUserIdOutput().id,
    //     },
    //     {
    //         permissions: ['READ'],
    //         type: 'Group',
