...
func TestProviderUpgrade(t *testing.T) {
	ptest.VerifyUpgrade(t).
		...
		WithProgram(filepath.Join("testdata", "resources")).
		WithResourceProviderServer(providerServer(t)).
		// Updating from the current baseline version generates an Update plan on the
		// provider, but not any of the resources. There are ProgramTest options like
		// AllowEmptyPreviewChanges that would allow the test to pass, but they stop
		// detecting resource Updates which is not acceptable. Perpahs there is a way to
		// AllowEmptyPreviewChanges in combination with manual gRPC log checks to get to
		// check only resources, and not the provider for upgrades.
		//
		// In the meanwhile verification relies on TestProviderUpgradeQuick in-memory test.
		Skip(ptest.UpgradeTestMode_PreviewOnly,
			"TODO[pulumi/pulumi-aws#2722] skip because of Updates on explicit provider").
		Run()
